...
class GlobalPowerLimitOptimizer(Callback):
    ...
    def __init__(
        ...
        try:
            for index in monitor.nvml_gpu_indices:
                ...
                command = "sudo nvidia-smi --id=" + str(index) + " --persistence-mode=1"
                # print(command)
    ...
    def on_step_begin(self) -> None:
        ...
        if isinstance(self.state, Ready):
            self.state.steps -= 1
            temp = self.state.next_power_limit
            ...
            return self.state.current_power_limit

        elif isinstance(self.state, Profiling):
            self.state.steps -= 1
            temp = self.state.current_power_limit
            if self.state.steps == 0:
                ...
                if current_power_limit_index == len(self.power_limits) - 1:
                    ...
                    self._set_power_limit(self.state.optimal_power_limit)
                    self._save_profile()
                    temp = self.state.optimal_power_limit
                else:
            ...
            return self.state.optimal_power_limit
    ...
    def _set_power_limit(self, power_limit: int) -> None:
        ...
        for handle in self.handles:
            ...
            # print(power_limit)
            command = "sudo nvidia-smi --id=" + str(int(gpu_id)) + " --power-limit=" + str(float(power_limit//1000))
            # print(command)
            ret = os.system(command=command)
